...
<template>
  ...
  <div class="flex justify-between items-center mx-14 mt-4">
    ...
    <router-link class="bg-green-500 p-3 text-white rounded-lg cursor-pointer" :to="{ path: '/admin/post/new' }">Item toevoegen +</router-link>
  </div>
  <div class="flex flex-wrap mx-14" :class="State.gettingPosts ? 'justify-center' : ''">
    <router-link v-if="!State.gettingPosts" v-for="item in State.data" :to=" {path: '/admin/post/' + item.id } ">
      <Post
          :name="item.name"
          :url="item.url"
          admin="true"
